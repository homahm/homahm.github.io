...
<style>
  ...
  .group-container.ra > .group-heading{ grid-column:1 / -1; margin-bottom:.5rem; }
  ...
  .group-container.ra img.img-fluid{
    width:100%; height:180px; object-fit:cover; border-radius:.5rem .5rem 0 0; display:block;
  }
  ...
  /* ===== Former lab members: tiny stacked cards, 5 per row, name only ===== */
  .group-container.former{
    display:grid;
    grid-template-columns:repeat(5,minmax(0,1fr));
    gap:.75rem;
  }
  .group-container.former > .group-heading{ grid-column:1 / -1; margin-bottom:.5rem; }
  .group-container.former .mini-card{ margin:0; text-align:center; padding:0; border:none; box-shadow:none; background:transparent; }
  .group-container.former .mini-img{
    width:100%; height:110px; object-fit:cover; border-radius:.5rem; display:block;
  }
  .group-container.former .mini-name{ font-size:.95rem; margin-top:.4rem; font-weight:600; }
...
{% comment %}
Special groups:
- Research Assistants → grid 4-up stacked.
- Former lab members → grid 5-up, photo + name only.
String match is done case-insensitively via `downcase`.
{% endcomment %}
  ...
  <div class="group-container
    ...
    {% if group | downcase == 'former lab members' %}
      {%- for member in members -%}
        <div class="mini-card">
          {%- comment -%} Link if inline=false {%- endcomment -%}
